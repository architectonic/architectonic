...
import {
  ...
  readJson, rel, resolveManagedPath, run, shortSha, style, writeManifest,
} from "./runtime.js";
...
export function inspectLayer(root, layer, item, { semantic = false } = {}) {
  ...
  let dir;
  try {
    dir = resolveManagedPath(root, item.path || `./${layer}`);
  } catch (error) {
    fail("managed_path", error.message);
    return {
      layer, dir: null, ok, checks, warnings,
...
function runInspection(tokens, semantic) {
  ...
  if (semantic) {
    ...
    if (manifest.layers?.rail) {
      const ledgerRelative = manifest.rail?.ledger_path || LAYERS.rail?.ledgerPath;
      if (!ledgerRelative) {
        rootChecks.push({ level: "fail", code: "missing_rail_binding", message: "rail is installed without one canonical ledger path" });
      } else {
        try {
          const ledger = resolveManagedPath(parsed.dir, ledgerRelative);
          if (!exists(ledger)) {
            rootChecks.push({ level: "fail", code: "missing_rail_ledger", path: ledgerRelative, message: `missing canonical rail ledger ${ledgerRelative}; run architectonic onboard --fix` });
          } else {
            const railDir = resolveManagedPath(parsed.dir, manifest.layers.rail.path);
            const validator = path.join(railDir, LAYERS.rail.validator);
            const validation = run(process.execPath, [validator, "validate", ledger, "--json"], { cwd: parsed.dir });
            if (validation.status !== 0) {
              let reason = (validation.stderr || validation.stdout || "").trim();
              try {
                const payload = JSON.parse(validation.stdout);
                reason = payload.errors?.join("; ") || payload.error || reason;
              } catch { /* retain process output */ }
              rootChecks.push({ level: "fail", code: "invalid_rail_ledger", path: ledgerRelative, message: `invalid canonical rail ledger: ${reason}` });
            }
          }
        } catch (error) {
          rootChecks.push({ level: "fail", code: "invalid_rail_binding", path: ledgerRelative, message: error.message });
        }
      }
    }
  }
  if (parsed.fix) {
    ...
    if (manifest.layers?.rail && !manifest.rail) manifest.rail = { ledger_path: `./${LAYERS.rail.ledgerPath}` };
...
export function mapCommand(tokens) {
  ...
  const rows = Object.entries(manifest.layers || {}).map(([layer, item]) => {
    const definition = LAYERS[layer];
    const dir = resolveManagedPath(parsed.dir, item.path || `./${layer}`);
    let protocol = null;
...
export function diff(tokens) {
  ...
  const result = git(resolveManagedPath(parsed.dir, item.path), ["diff", "--"]);
