...
function runInspection(tokens, semantic) {
  ...
  if (semantic) {
    ...
    if (!exists(path.join(parsed.dir, "ONBOARDING.md"))) rootChecks.push({ level: "fail", code: "missing_onboarding", message: "missing ONBOARDING.md; run architectonic onboard --fix" });
    if (!exists(path.join(parsed.dir, "organization", "README.md"))) rootChecks.push({ level: "fail", code: "missing_organization", message: "missing organization/README.md; run architectonic onboard --fix" });
    for (const layer of required) {
      const localEntry = LAYERS[layer]?.localEntry;
      if (localEntry && !exists(path.join(parsed.dir, localEntry))) {
        rootChecks.push({ level: "fail", code: "missing_local_entry", layer, message: `missing local ${layer} entry ${localEntry}; run architectonic onboard --fix` });
      }
    }
  }
...
export function mapCommand(tokens) {
  ...
  println();
  println(style.dim("  Read the local organization entry first, then its upstream contract when the governing boundary matters."));
}
