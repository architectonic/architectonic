...
export function welcome() {
  ...
  println(`    ${style.cyan("npx architectonic onboard --dir my-organization")}`);
  ...
  const rows = [
    ...
    ["onboard", "route document-guided human interviews into local organization files"],
    ["map", "show local entries, upstream contracts, and their purpose"], ["verify", "run semantic protocol conformance"],
...
export function helpCommand(name) {
  const guides = {
    ...
    onboard: "architectonic onboard — inspect or repair the organization-owned document scaffold\n\nUsage:\n  npx architectonic onboard [--dir <path>] [--fix] [--json]\n\nRead ONBOARDING.md, inspect sources first, ask targeted questions only where material gaps remain, and record explicit answers in organization/.",
    map: "architectonic map — show organization-owned entries and upstream contracts\n\nUsage:\n  npx architectonic map [--dir <path>] [--json]",
