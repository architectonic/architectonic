...
export const PROFILE_METADATA = PROFILE_DOCUMENT.profile_metadata || {};
export const NEED_RULES = PROFILE_DOCUMENT.need_rules || [];
export const REPO_URL = PKG.repository?.url?.replace(/^git\+/, "").replace(/\.git$/, "")
  || "https://github.com/architectonic/architectonic";
export const COMMANDS = [
  "init", "add", "recommend", "onboard", "graph", "list", "doctor", "verify", "map", "status",
  "diff", "update", "upgrade", "remove", "agent",
];
...
export function normalizePath(value) { return value.replace(/\\/g, "/"); }
...
function nearestExistingParent(target) {
  ...
  while (!exists(current)) {
    ...
    if (parent === current) throw new Error(`cannot resolve an existing parent for managed path: ${target}`);
    current = parent;
  }
  return current;
}

export function resolveManagedPath(root, candidatePath) {
  if (typeof candidatePath !== "string" || !candidatePath.trim()) throw new Error("managed path must be a non-empty string");
  const absoluteRoot = path.resolve(root);
  if (!exists(absoluteRoot)) throw new Error(`workspace root does not exist: ${absoluteRoot}`);
  const realRoot = fs.realpathSync(absoluteRoot);
  const absoluteCandidate = path.resolve(absoluteRoot, candidatePath);
  if (!isContained(absoluteRoot, absoluteCandidate)) throw new Error(`managed path escapes the workspace: ${candidatePath}`);
  const realBoundary = fs.realpathSync(nearestExistingParent(absoluteCandidate));
  if (!isContained(realRoot, realBoundary)) throw new Error(`managed path escapes the workspace through a symlink: ${candidatePath}`);
...
export function ensureSource(source) {
  ...
  if (source === "npm") {
    ...
    if (run(npm.command, npm.args).status !== 0) throw new Error("npm is required on PATH for --source npm.");
...
export function hashDirectory(directory) {
  ...
  const visit = (dir) => {
    for (const entry of fs.readdirSync(dir, { withFileTypes: true }).sort((a, b) => a.name.localeCompare(b.name))) {
      if ([".git", "node_modules", ".architectonic"].includes(entry.name)) continue;
...
export function parse(tokens) {
  const parsed = {
    ...
    need: null, format: "both", includeContracts: false,
  };
  const valueOptions = new Map([
    ["--dir", "dir"], ["--out", "dir"], ["--source", "source"], ["--preset", "preset"],
    ["--spec", "spec"], ["--output", "output"], ["--need", "need"], ["--format", "format"],
  ]);
  for (let index = 0; index < tokens.length; index += 1) {
    ...
    else if (token === "--include-contracts") parsed.includeContracts = true;
    ...
    } else if (["--dir=", "--out=", "--source=", "--preset=", "--spec=", "--output=", "--need=", "--format="].some((prefix) => token.startsWith(prefix))) {
...
export function profileName(input) { return PROFILE_ALIASES[input] || input; }

function resolvePresetPart(part) {
  const name = profileName(part);
  if (PROFILES[name]) return { name, layers: [...PROFILES[name]], profiles: [name], features: profileFeatures([name]) };
  if (LAYERS[name]) return { name, layers: [name], profiles: [], features: [] };
  throw new Error(`Unknown layer or profile: ${part}.`);
}

export function resolvePreset(input) {
  const raw = input || "full";
  const exact = profileName(raw);
  if (PROFILES[exact]) return { name: exact, layers: [...PROFILES[exact]], profiles: [exact], metadata: PROFILE_METADATA[exact] || {}, custom: false };
  if (LAYERS[exact]) return { name: exact, layers: [exact], profiles: [], metadata: PROFILE_METADATA[exact] || {}, custom: false };
  if (/[+,]/.test(raw)) {
    const parts = raw.split(/[+,]/).map((value) => value.trim()).filter(Boolean);
    if (!parts.length) throw new Error("Custom preset contains no layers or profiles.");
    const plans = parts.map(resolvePresetPart);
    return {
      name: `custom:${parts.join("+")}`,
      layers: unique(plans.flatMap((plan) => plan.layers)),
      profiles: unique(plans.flatMap((plan) => plan.profiles || [])),
      metadata: { summary: "Custom composition", features: unique(plans.flatMap((plan) => plan.features || [])) },
      custom: true,
    };
  }
  throw new Error(`Unknown preset: ${input}. Use a layer, a named profile, or a + separated composition.`);
}

export function resolveAddPlan(targets) {
  const layers = [];
  const profiles = [];
  for (const target of targets.length ? targets : ["full"]) {
    ...
    if (PROFILES[name]) {
      layers.push(...PROFILES[name]);
      profiles.push(name);
      continue;
    }
    if (/[+,]/.test(target)) {
      const plan = resolvePreset(target);
      layers.push(...plan.layers);
      profiles.push(...(plan.profiles || []));
      continue;
    }
    throw new Error(`Unknown layer or profile: ${target}.`);
  }
  return { layers: unique(layers), profiles: unique(profiles), features: profileFeatures(unique(profiles)) };
}

export function resolveAddTargets(targets) { return resolveAddPlan(targets).layers; }

export function profileFeatures(profiles = []) {
  return unique(profiles.flatMap((profile) => PROFILE_METADATA[profile]?.features || []));
}
