...
import {
  PROFILES, dirty, exists, finishJson, git, gitHead, hashDirectory, icon, load, padEnd, parse,
  pkgVersion, println, profileFeatures, rel, resolveManagedPath, run, writeManifest,
} from "./runtime.js";
...
function updateGitLayer(root, layer, item, dryRun) {
  const dir = resolveManagedPath(root, item.path);
...
function upgradeNpmLayer(root, layer, item, dryRun) {
  const currentDir = resolveManagedPath(root, item.path);
...
export function remove(tokens) {
  ...
  const dir = resolveManagedPath(parsed.dir, item.path);
  ...
  if (layer === "rail") delete manifest.rail;
  manifest.required_layers = (manifest.required_layers || []).filter((name) => name !== layer);
  manifest.profiles = (manifest.profiles || []).filter((profile) => !(PROFILES[profile] || []).includes(layer));
  manifest.features = profileFeatures(manifest.profiles);
...
export function agentCommand(tokens) {
  ...
  const agentsDir = resolveManagedPath(parsed.dir, item.path);
