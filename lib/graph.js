...
import {
  exists, finishJson, load, normalizePath, parseDirArg, println, rel, style, writeJson,
} from "./runtime.js";
...
function buildGraph(workspace, manifest, includeContracts) {
  ...
  if (includeContracts) {
    for (const [layer, item] of Object.entries(manifest.layers || {})) {
      roots.push({ absolute: path.resolve(workspace, item.path || `./${layer}`), scope: `contract:${layer}` });
    }
  }
