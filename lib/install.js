...
export function init(tokens) {
  ...
  fs.mkdirSync(dir, { recursive: true });
  fs.writeFileSync(path.join(dir, "README.md"), `# ${name || path.basename(dir)}\n\nHuman–AI organization initialized with Architectonic.\n\nRun \`npx architectonic map\` to locate the installed operating knowledge.\n`, "utf8");
  fs.writeFileSync(path.join(dir, "AGENTS.md"), `# Agent instructions\n\n1. Run \`npx architectonic map\` before broad repository search.\n2. Read the relevant canonical layer entry before changing durable artifacts.\n3. Treat source artifacts as evidence; preserve assumptions and unknowns explicitly.\n4. Verify authority, acceptance criteria, and stopping conditions before material action.\n`, "utf8");
  ...
  println();
  println(`  ${style.dim("Next:")} npx architectonic verify --dir ${JSON.stringify(dir)}`);
