...
import {
  LAYERS, PROTOCOL_VERSION, VERSION, ensureSource, exists, gitHead, hashDirectory, icon, layerPath, manifestPath,
  npmInvocation, packageSpec, parse, pkgName, pkgVersion, println, profileFeatures, readManifest, rel, repoUrl, resolvePreset, run,
...
9. When Rail is installed, read the one ledger named by \`architectonic.json\`; do not create a parallel queue, now file, status file, or handoff ledger.
10. Verify authority, acceptance criteria, review gates, and stopping conditions before material action.
11. Preserve unresolved questions instead of inventing answers to complete a template.
`;
...
export function extractPackedLayer(layer, parentDir) {
  ...
  try {
    const npm = npmInvocation(["pack", spec, "--pack-destination", temp]);
    const packed = run(npm.command, npm.args, { cwd: parentDir });
...
function rollbackFailedInit(dir, existedBefore, initialEntries) {
  const errors = [];
  try {
    if (exists(dir)) {
      for (const entry of fs.readdirSync(dir)) {
        if (initialEntries.has(entry)) continue;
        try {
    ...
    errors.push(`inspect target: ${error.message}`);
  }

  if (!existedBefore && exists(dir)) {
    try {
      if (fs.readdirSync(dir).length === 0) fs.rmdirSync(dir);
    } catch (error) {
      errors.push(`remove target: ${error.message}`);
    }
  }
  return errors;
}
...
export function init(tokens) {
  ...
  const existedBefore = exists(dir);
  const initialEntries = new Set(existedBefore ? fs.readdirSync(dir) : []);
  if (initialEntries.size > 0) throw new Error(`Refusing to initialize into a non-empty directory: ${dir}`);
  ...
  try {
    ...
    const cleanupErrors = rollbackFailedInit(dir, existedBefore, initialEntries);
    if (cleanupErrors.length) {
      throw new Error(`${primary.message}\nInitialization cleanup also failed: ${cleanupErrors.join("; ")}`, {
        cause: primary,
      });
    }
    throw primary;
